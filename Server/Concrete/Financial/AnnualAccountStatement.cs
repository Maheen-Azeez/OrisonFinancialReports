...
{
    public class AnnualAccountStatement : IAccountStatement
    {
        ...
        public async Task<List<AcctStmt>> GetAccountStatement(string key, DynamicParameters parameters, IDapperManager dapperManager)
        {
            try
            {
                parameters.Add("Criteria", "Year", DbType.String);
                var result =  Task.Run(() =>
                {
                    var acctStmtDtos = dapperManager.GetAll<AnnualStatementDto>(
            ...
            {
                throw new StoredProcedureRelatedException("Yearly statement configuration is still pending. Contact the administrator, please.", ex.InnerException);
